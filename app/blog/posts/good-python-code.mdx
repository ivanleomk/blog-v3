...
I've always used python for my interviews but it wasn't until about 6 months ago that I started to use python for all sorts of projects - from simple machine learning scripts to full blown applications. In the proccess, I've learnt a lot about what constitutes good python code and I hope that you'll find the tips in this article useful in your own projects.
...
2. taking time to learn about common libraries
3. writing simpler functions
4. being a bit lazier - earn the abstraction
5. decouple your implementation

## Use the classmethod decorator

The more complex your code's logic, the more likely it is to have a strange bug. In these cases, I find the classmethod decorator to be immensely useful. I think the best example of this was from a talk by [James Powell from Don't Use This Code](http://www.dontusethiscode.com) where he uses `pandas` as an example of a library that uses classmethods well.

If you've ever used the library, you'll be familiar with the following methods.
...
A better solution instead is to parameterize the scoring function itself. If you look at the different functions we've defined, they all take in a single label. We're also doing the same thing in each function, which is to score the predictions with respect to a specific output.
...
def slice_predictions_at_k(k: int, score):
