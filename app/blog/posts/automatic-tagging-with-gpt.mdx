...
```py class Status(Enum): CLOSED = "Closed" TEMPORARILY_CLOSED = "Temporarily Closed"
OPEN = "Open"
...
```python class Tags(BaseModel): categories: List[str] def generate_categories(keywords):
...
```python import streamlit as st import pandas as pd from pydantic import BaseModel
